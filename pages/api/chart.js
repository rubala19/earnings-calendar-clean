...
// Returns a normalised response for the SVG chart component.
...
export default async function handler(req, res) {
  ...
  if (!config) return res.status(400).json({ error: `Invalid range. Use: ${Object.keys(VALID_RANGES).join(', ')}` });
  ...
  try {
    ...
    // Build clean point array — skip bars with null close (pre/post market gaps)
    const points = [];
    ...
    const priceMin    = Math.min(...allCloses);
    const priceMax    = Math.max(...allCloses);

    dbg(`[chart] ${ticker} ${points.length} bars, last=${lastClose.toFixed(2)}`);

    // Cache 5 minutes — price data changes frequently
